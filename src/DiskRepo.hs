...
module DiskRepo (insert, findAll, removeById, updateIfCurrent) where
...
import           Crud                       (CrudModificationException (..),
                                             CrudReadException (..),
                                             CrudWriteException (..),
...
-- FUNCTIONS
insert :: (DiskFileStorageConfig crudConfig, Content a) => crudConfig -> a -> EitherT CrudWriteException IO StorageId
insert config nc = do
...
findAll :: (DiskFileStorageConfig crudConfig, Content a) => crudConfig -> EitherT CrudReadException IO [EitherT CrudReadException IO (Identifiable a)]
findAll config = do
...
removeById :: DiskFileStorageConfig confType => confType -> String -> EitherT CrudWriteException IO ()
...
writeContentToFile :: (DiskFileStorageConfig crudConfig, Content a) => crudConfig -> a -> String -> EitherT CrudWriteException IO StorageId
...
readItemFromFile :: (DiskFileStorageConfig crudConfig, Content a) => crudConfig -> SimpleFileName -> EitherT CrudReadException IO (Identifiable a)
readItemFromFile config file = do
    stringToParse <- (readFileImpl . prefixWithStorageDir config) file
    firstEitherT (parsingExceptionToCrudException file) $ parseString stringToParse

parseString :: Content a => ByteString -> EitherT ParsingException IO (Identifiable a)
parseString strToParse = firstEitherT (ParsingException strToParse)
